...
data_lists = [["orgs", orgs], ["places", places], ["homelands", homelands], ["provinces", provinces], ["hrvs", hrvs]]
for name, data_list in data_lists:
    change_sessions(name, data_list)

selected_orgs = st.sidebar.multiselect("Select Organization(s)", st.session_state.orgs)

selected_places = st.sidebar.multiselect("Select Place(s)", st.session_state.places)
selected_homelands = st.sidebar.multiselect("Select Homeland(s)", st.session_state.homelands)
selected_provinces = st.sidebar.multiselect("Select Province(s)", st.session_state.provinces)
selected_hrvs = st.sidebar.multiselect("Select HRV(s)", st.session_state.hrvs)
...
res = full_data
if selected_orgs:
    res = filter_df(full_data, "org", selected_orgs)
    # st.write(res)
if selected_places:
